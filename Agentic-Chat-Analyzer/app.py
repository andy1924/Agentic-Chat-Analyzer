...
if 'analysis_df' in locals() and not analysis_df.empty:
    ...
    col1.metric(label="Overall Health Score", value=f"{avg_health_score}/100", help="Median relationship health across the network.")
    col2.metric(label="Network Interaction Balance", value=f"{sent_pct}% / {recv_pct}%", help="Percentage of valid messages sent by you vs received.")
    col3.metric(label="Avg Inactivity Gap", value=f"{avg_inactive_days} days", help="Median inactivity gap across all networks.")
